...
export class CrearTiendaUseCase {
  ...
  async execute(dto: CreateTiendaRequestDto, idUsuario: string): Promise<TiendaResponseDto> {
    ...
    let vendedor = await this.vendedorRepo.findOne({ where: { id_vendedor: idUsuario } });
    if (!vendedor) {
